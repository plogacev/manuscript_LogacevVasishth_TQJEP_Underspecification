...
parameters
{
	...
	real<lower=0, upper=1> p_uspec_hyper[5];
...
model
{
	...
	for(j in 1:n_subj)
	{
		increment_log_prob( beta_log_hyper_mix(p_uspec_subj[j], p_uspec_hyper) );
		//increment_log_prob( beta_log_hyper(p_uspec_unamb_subj[j], p_uspec_unamb_hyper) );
		increment_log_prob( beta_log_hyper(p_arbitrary_attachment_subj[j], p_arbitrary_attachment_hyper) );
		increment_log_prob( beta_log_hyper(p_att_n1_subj[j], p_att_n1_hyper) );
		increment_log_prob( beta_log_hyper(p_retrieval_fail_subj[j], p_retrieval_fail_hyper) );
		increment_log_prob( beta_log_hyper(p_guess_yes_subj[j], p_guess_yes_hyper) );
